...
const ManageTasks = ({ handleBackToMenu }) => {
  ...
  const handleDownloadReport = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/api/rep/admin-reports`, { responseType: 'blob' });
  ...
  return (
    <div className='w-full relative h-screen p-2'>
      <div className='flex absolute z-20 items-center sm:hidden py-2'>
      ...
      <div className='w-full  overflow-y-auto max-sm:pt-10 scrollbar sm:pt-12 h-full'>
        <div className='flex max-lg:flex-col justify-between lg:items-center'>
          ...
          <div onClick={handleDownloadReport} className='flex gap-2 max-lg:hidden hover:border hover:border-blue-500 rounded bg-green-200 p-1 items-center justify-center'>
            <GrDocumentText className='text-slate-500 cursor-pointer' />
            <p className='text-sm font-semibold'>Download Report</p>
          </div>
        </div>
        {
              ...
              <div className='grid grid-cols-3 gap-2 mt-3 max-md:grid-cols-2 max-sm:grid-cols-1 w-full'>
                {
                  filteredTasks?.map((t,i)=> {
                    return (
                      <div onClick={()=> navigate(`/dashboard/admin/update-task/${t?.id}`)} key={i} style={{ boxShadow: '0px 0px 5px 0px #94a3b8' }} className='w-full cursor-pointer'>
