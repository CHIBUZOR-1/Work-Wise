...
const TeamPanel = ({ handleBackToMenu }) => {
  ...
  return (
    <div className='w-full h-full relative p-2 scrollbar overflow-y-auto'>
      ...
      <div className='h-full w-full px-1 py-2 max-sm:pt-12 sm:pt-14 overflow-y-auto scrollbar'>
        <div style={{ boxShadow: '0px 0px 5px 0px #94a3b8' }} className='w-full  p-2'>
          ...
          <div className='p-1 grid grid-cols-4 max-md:grid-cols-2 gap-1 items-center justify-between font-normal text-slate-600'>
            ...
            <p className='p-1 border-l-4 max-sm:text-sm border-l-red-400'> <span className='font-semibold border p-1'>{allSummaries?.pending}</span> Pending Tasks</p>
            <p className='p-1 border-l-4 max-sm:text-sm border-l-yellow-400'> <span className='font-semibold border p-1'>{allSummaries?.inProgress}</span> In Progress</p>
            <p className='p-1 border-l-4 max-sm:text-sm border-l-green-400'> <span className='font-semibold border p-1'>{allSummaries?.completed}</span> Completed Tasks</p>
          </div>
        </div>
        {
            load && (
        ...
        }
        {
            !load && allTasks.length === 0 && (
              <div className={`${allTasks.length > 0 && 'hidden'} flex items-center justify-center p-3`}>
                <p className='font-semibold text-slate-400'>You have no tasks yet.</p>
              </div>
            )
        }
        {
            !load && allTasks.length > 0 && (
              <div className='w-full'>
                <div className={`grid grid-cols-2 max-md:grid-cols-1 ${allTasks.length === 0 && 'hidden'} w-full pt-2 gap-1`}>
                  <div style={{ boxShadow: '0px 0px 5px 0px #94a3b8' }} className='p-2 flex flex-col'>
                    <h3 className='font-semibold max-sm:text-sm'>Task Distribution</h3>
                    <TaskStatusChart taskz={allTasks}/>
                  </div>
                  <div style={{ boxShadow: '0px 0px 5px 0px #94a3b8' }} className='p-2'>
                    <h3 className='font-semibold max-sm:text-sm'>Task Priority</h3>
                    <TaskPriorityChart taskd={allTasks}/>
                  </div> 
                </div>
                <div className={`w-full p-1 ${allTasks.length === 0 && 'hidden'}`}>
                          ...
                          return(
                            <tr key={idx} className=''>
                              ...
                              <td className=' p-1'>
                                {
                                  tsk.priority === 'Low' && (
                                  <p className='p-1 w-full max-sm:text-xs bg-blue-200 font-semibold rounded-md text-blue-600 text-center border'>{tsk.priority}</p> 
                                  )
                                }
                                {
                                  tsk.priority === 'Medium' && (
                                  <p className='p-1 w-full max-sm:text-xs bg-yellow-200 font-semibold rounded-md text-yellow-500 text-center border'>{tsk.priority}</p> 
                                  )
                                }
                                {
                                  tsk.priority === 'High' && (
